...
CREATE TABLE `user` (
  ...
  `FINANTIALMONTHS_DATE` date DEFAULT NULL,
  PRIMARY KEY (`ID`),
  KEY `FK_user_FINANTIALMONTHS_DATE` (`FINANTIALMONTHS_DATE`),
  CONSTRAINT `FK_user_FINANTIALMONTHS_DATE` FOREIGN KEY (`FINANTIALMONTHS_DATE`) REFERENCES `finantial_month` (`DATE`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `user` (`ID`,`USERNAME`,`PASSWORD`,`FULLNAME`,`FINANTIALMONTHS_DATE`) VALUES 
 (1,'rmpestano','rmp','Rafael M. Pestano',NULL);
